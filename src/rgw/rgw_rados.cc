...
int RGWRados::cls_obj_usage_log_trim(string& oid, string& user, string& subuser, uint64_t start_epoch, uint64_t end_epoch, int yes_i_really_mean_it, bool subuser_specified)
{
  rgw_raw_obj obj(get_zone_params().usage_log_pool, oid);
  ...
  ObjectWriteOperation op;
  cls_rgw_usage_log_trim(op, user, subuser, start_epoch, end_epoch, yes_i_really_mean_it, subuser_specified);
