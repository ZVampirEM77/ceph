...
void RGWPutBL::execute()
{
  ...
  if (status->is_enabled()) {
     ...
     if (tbucket_name != s->bucket_name) { // if target bucket isn't same as source bucket,
       ...
       // AWS S3
       // It will return error if ldg is not granted write and 
       // read_acp when enable bucket logging of source bucket and
       // target bucket is not the same one with source bucket.
