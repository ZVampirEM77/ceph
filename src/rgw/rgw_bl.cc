...
#define MAX_BL_LIST_ENTRIES 100
  do {
    int ret = cls_rgw_lc_list(store->bl_pool_ctx,
    ...
    for (auto iter = entries.begin(); iter != entries.end(); ++iter) {
      ...
      ret = cls_rgw_lc_set_entry(store->bl_pool_ctx, obj_names[index], entry);
      if (ret < 0) {
...
int RGWBL::bucket_bl_post(int index, int max_lock_sec, cls_rgw_lc_obj_head& head,
                          pair<string, int>& entry, int& result)
{
  ...
  do {
    ...
    if (result == -ENOENT) {
      ret = cls_rgw_lc_rm_entry(store->bl_pool_ctx, obj_names[index],  entry);
      if (ret < 0) {
                            ...
                            map<string, int> *progress_map)
{
  ...
  for(; index < max_objs; index++) {
    map<string, int > entries;
    int ret = cls_rgw_lc_list(store->bl_pool_ctx, obj_names[index],
...
int RGWBL::process(int index, int max_lock_secs)
{
  ...
  do {
    ...
    ret = cls_rgw_lc_get_head(store->bl_pool_ctx, obj_names[index], head);
    if (ret < 0) {
    ...
    ret = cls_rgw_lc_get_next_entry(store->bl_pool_ctx, obj_names[index],
                                    head.marker, entry);
    if (ret < 0) {
    ...
    ret = cls_rgw_lc_put_head(store->bl_pool_ctx, obj_names[index],  head);
    if (ret < 0) {
    ...
    ret = bucket_bl_post(index, max_lock_secs, head, entry, ret);
    return 0;
