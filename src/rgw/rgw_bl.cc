...
std::map<const string, int> rgw_perm_map = boost::assign::map_list_of
      ("READ", RGW_PERM_READ)
...
int RGWBL::bucket_bl_deliver(string opslog_obj, rgw_bucket target_bucket,
...
#define MAX_OPSLOG_UPLOAD_ENTRIES 10000
  max_entries = cct->_conf->rgw_bl_max_ops_log_entries;
  if (max_entries < 0)
  ...
  if (ret == -ENOENT) {
    ...
    if (ret < 0) {
      ...
      if (ret == 0) { 
         ...
         int rgw_permission = RGW_PERM_NONE;
         ...
         for (auto grant_iter = bl_grant.begin(); (grant_iter != bl_grant.end()) && (acl_nums < RGW_ACL_MAX_NUMS); grant_iter++) {
           ...
           if (acl_grantee_type == ACL_TYPE_CANON_USER) {
             ...
             std::string display_name = grant_iter->get_display_name();
             target_grant.set_canon(canonical_user, display_name,
		                    rgw_permission);
